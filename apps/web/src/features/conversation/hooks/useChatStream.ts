 ...
 * frame `data: <json>\n\n`. Turn có thể pause chờ duyệt (`approval_required`, ADR-0014) — gọi
 * `respond()` để duyệt/từ chối, tiếp tục đúng turn đó qua `POST .../chat/approve`. */
export function useChatStream(conversationId: number) {
        ...
        while (frameEnd !== -1) {
          ...
          if (event.type === 'delta') {
            ...
            setState((prev) => ({
              ...
              toolCallName: null,
              approvalRequest: { toolName: event.tool_name, arguments: event.arguments },
            }));
  ...
  const respond = useCallback(
    async (decision: 'approve' | 'reject') => {
      setState((prev) => ({ ...prev, status: 'streaming', approvalRequest: null }));

      let response: Response;
      try {
        response = await fetch(
          `${ENV.apiBaseUrl}${endpoints.conversations.chat(conversationId)}/approve`,
          {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
